...
import api from "../services/apiToken"; // ✅ dùng axios instance có refresh token
...
export default function AiJournalismPage() {
  ...
  // 🧩 Mở 1 cuộc thi → load bài của user + leaderboard
  async function openContest(contest) {
    ...
    try {
      if (user?.id) {
        const res1 = await api.get(`/journalism/entries/student/${user.id}`);
            ...
            <table className="min-w-full border border-gray-200 rounded-lg overflow-hidden">
              ...
              <tbody>
                {leaderboard.map((item, i) => (
                  <tr
                    key={item.entryId}
                    className={`border-t ${i < 3 ? "bg-yellow-50" : "hover:bg-gray-50"} transition-all duration-200`}
                  >
